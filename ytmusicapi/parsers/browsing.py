        ...
        for result in results:
            ...
            if resultType == "artist":
                ...
                search_result["itemCount"] = nav(
                    flex_item, [default_offset + has_author * 2, "text"]
                ).split(" ")[0]
                search_result["author"] = (
                    None if not has_author else nav(flex_item, [default_offset, "text"])
                )
                ...
                if "menu" in data:
                    ...
                    if toggle_menu:
                        search_result["feedbackTokens"] = parse_song_menu_tokens(
                            toggle_menu
                        )
            ...
            if resultType in ["song", "video", "album"]:
                ...
                has_offset = resultType == "album" or (
                    default_offset and bool(search_result["videoId"])
                )
        ...
        for i, category in enumerate(categories):
            ...
            if len(data) > 0:
                ...
                if "navigationEndpoint" in nav(data[0], CAROUSEL_TITLE):
                    artist[category]["browseId"] = nav(
                        data[0], CAROUSEL_TITLE + NAVIGATION_BROWSE_ID
                    )
                ...
                artist[category]["results"] = parse_content_list(
                    data[0]["contents"], categories_parser[i]
                )
